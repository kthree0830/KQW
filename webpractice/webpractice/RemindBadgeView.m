...
#import "RemindBadgeView.h"
@interface RemindBadgeView()
@property (strong, nonatomic) CAShapeLayer *shapeLayer;
@property (strong, nonatomic) UIView *smallBadgeView;//和self一样的图
@end
@implementation RemindBadgeView
...
-(void)drawRect:(CGRect)rect
{
    ...
    UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:rectToDraw byRoundingCorners:(UIRectCorner)UIRectCornerAllCorners cornerRadii:CGSizeMake(10, 10)];
    //     绘制背景色,背景阴影
    CGContextSaveGState(ctx);
    {
        ...
        CGContextSetShadowWithColor(ctx, CGSizeZero, 10, [UIColor clearColor].CGColor);
...
-(void)handleTap:(UITapGestureRecognizer*)tap
{
    ...
    if ([self.delegate respondsToSelector:@selector(removeRemindBadgeView:)]) {
        [self.delegate removeRemindBadgeView:self];
    }
...
- (void)handlePan:(UIPanGestureRecognizer *)pan{
    ...
    if (dist < self.maxDistance) {
        ...
        self.smallBadgeView.bounds = CGRectMake(0, 0, 50, 50);
